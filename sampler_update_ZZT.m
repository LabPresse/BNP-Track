function [sample_K,sample_X,sample_Y,sample_Z,rec] = sampler_update_ZZT( ...
...
for rep = 1 : poissrnd(2*2*params.N*M_idx)
    ...
    propos_z_idx(n_idx) = - propos_z_idx(n_idx);

    if (propos_z_idx(sample_K_idx(m))<params.Z_prior_min) || (propos_z_idx(sample_K_idx(m))>params.Z_prior_max)
        log_a = -inf;
    else
